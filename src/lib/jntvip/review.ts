...
export async function getReviewerName(): Promise<string> {
  const row = await db.meta.get(REVIEWER_NAME_META_KEY)
  return (row?.value as string | undefined) ?? ''
}

export async function setReviewerName(name: string): Promise<void> {
  await db.meta.put({ key: REVIEWER_NAME_META_KEY, value: name })
}
...
async function setManualStatus(matchId: number, manualStatus: JntVipManualStatus, action: string, reviewedBy: string, note: string | null) {
  const existing = await db.jntVipMatches.get(matchId)
  if (!existing) throw new Error('Match not found.')
  const previousValue = { manualStatus: existing.manualStatus, status: existing.status }
  const reviewDate = new Date().toISOString()
  await db.jntVipMatches.update(matchId, { manualStatus, reviewedBy: reviewedBy || existing.reviewedBy, reviewDate, notes: note ?? existing.notes })
  await runJntVipReconciliation()
  const after = await db.jntVipMatches.get(matchId)
...
export async function addNote(matchId: number, note: string, reviewedBy: string): Promise<void> {
  ...
  await db.jntVipMatches.update(matchId, { notes: note, reviewedBy: reviewedBy || existing.reviewedBy, reviewDate: new Date().toISOString() })
...
export async function linkManually(
  ...
  const shipment = await db.jntVipShipments.get(shipmentId)
  ...
  const matchId = await db.jntVipMatches.add(draft)
  await runJntVipReconciliation()
